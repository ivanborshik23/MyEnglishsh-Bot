...
from database import init_db, add_common_words, async_session, User, Word, UserWord, get_or_create_user
...
async def command_start(message: types.Message):
    ...
    await message.answer(
        f"Привет, {message.from_user.first_name}! 👋\nДавай учить английский. Нажми 'Дальше', чтобы начать.",
...
async def next_card(message: types.Message, state: FSMContext):
    ...
    async with async_session() as session:
        ...
        others = random.sample([w for w in user_words if w.id != target.id], k=min(3, len(user_words)-1))
...
async def delete_word(message: types.Message, state: FSMContext):
    ...
    if not target:
        await message.answer("Сначала нажми 'Дальше', чтобы появилось слово.")
        return

    user = await get_or_create_user(message.from_user.id)
    async with async_session() as session:
        word_res = await session.execute(select(Word).where(Word.target_word == target))
        ...
        if word:
            ...
            await session.commit()
            await message.answer(f"Слово '{target}' удалено.", reply_markup=get_main_keyboard())
